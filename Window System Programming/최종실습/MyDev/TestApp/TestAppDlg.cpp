...
BEGIN_MESSAGE_MAP(CTestAppDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_START_BTN, &CTestAppDlg::OnBnClickedStartBtn)
	ON_BN_CLICKED(IDC_SUSPEND_BTN, &CTestAppDlg::OnBnClickedSuspendBtn)
	ON_BN_CLICKED(IDC_END_BTN, &CTestAppDlg::OnBnClickedEndBtn)
END_MESSAGE_MAP()
...
HANDLE g_hMonitor;
DWORD WINAPI ThreadMonitor(LPVOID p)
{
	int x = 0;
	int y = 0;
	CString strTemp;
...
void CTestAppDlg::OnBnClickedStartBtn()
{
	DWORD ID;
	if (::InitMyDev(5))
	{
		g_hMonitor = CreateThread(0, 0, ThreadMonitor, 0, 0, &ID);
	}
}


void CTestAppDlg::OnBnClickedSuspendBtn()
{
...
void CTestAppDlg::OnBnClickedEndBtn()
{
	::ReleaseMyDev();
