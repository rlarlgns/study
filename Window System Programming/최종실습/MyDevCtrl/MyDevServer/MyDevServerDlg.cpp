...
CMyDevServerDlg::CMyDevServerDlg(CWnd* pParent /*=NULL*/)
...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST1, m_ctrlDataBox);
}
...
BEGIN_MESSAGE_MAP(CMyDevServerDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_START_BTN, &CMyDevServerDlg::OnBnClickedStartBtn)
END_MESSAGE_MAP()
...
DWORD WINAPI ListenThread(LPVOID p)
{
	for (;;)
	{
		// 1. 파이프 생성
		HANDLE hPipe;
		hPipe = CreateNamedPipe(_T("\\\\.\\pipe\\mydevdatapipe"),
...
void CMyDevServerDlg::OnBnClickedStartBtn()
{
	g_pListBox = &m_ctrlDataBox;

	CloseHandle(CreateThread(NULL, 0, ListenThread, NULL, 0, NULL));
}
